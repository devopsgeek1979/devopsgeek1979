...
(function () {
  ...
  function renderRepo(repo) {
    const desc    = repo.description ? repo.description : '<em>No description</em>';
    const lang    = repo.language    ? `<span class="repo-lang">${langDot(repo.language)}${repo.language}</span>` : '';
    ...
    try {
      ...
      loading.innerHTML = `
        <div class="notice" style="text-align:left">
          <strong>Could not load repositories.</strong><br>
          ${err.message}<br>
          <a href="https://github.com/devopsgeek1979?tab=repositories" target="_blank" rel="noopener noreferrer">
